...
def _write_instance_ini(ini_path, db_path):
    """写 eyeterm 实例 ini：app_data=0 锚定数据目录；db_location 用官方键指定
    数据库文件（1.4.1 命令行无 -database 参数，传了会弹命令行帮助窗口）。"""
    try:
        lines = list(_FS_INI_LINES)
        if db_path:
            lines.append("db_location=%s" % db_path)
...
def start_everything():
    ...
    if not _write_instance_ini(ini_path, db_path):
        return {"success": False, "error": "ini_write_failed",
                "hint": "实例配置写入失败（数据目录不可写）"}
    try:
        proc = subprocess.Popen(
            [exe, "-instance", "eyeterm", "-config", ini_path, "-startup"],
