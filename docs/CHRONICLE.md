...
| 48 | 2026-09-10 | 网络监测配置 IP 脱敏清扫 | 真实业务 IP 清换中性示例 + 双层断言防回归门禁 |
| 47 | 2026-09-10 | 网络压测配置与可视化重塑 | 包长分档文本框 + 时长档位扩充（clamp 300）+ 分阶段进度条动画 |
| 50 | 2026-09-10 | route_nodes v5 基线勘误与扩充 | zone 语义纠错（终端区）+ 互联网出口节点（数据维护） |
...
#### 2026-09-10 · 网络监测配置 IP 脱敏清扫（#48）
- 事件：路由追踪 placeholder 等真实业务 IP 全量清换中性示例（net-doctor `88e784d` / 主仓库 `67db905`）；双层断言固化（文件级 grep + 渲染级 placeholder/title 检查）防回归。
- 意义：前端 IP 脱敏规范（#20）在网络监测配置模块的补全，脱敏从一次性清理升级为门禁防回归。
- 考证：net-doctor 提交 `88e784d`（2026-09-10 21:31）；主仓库提交 `67db905`（21:31）；E2E 226/226 来源：会话记忆（2026-09-10）。

#### 2026-09-10 · 网络压测配置与可视化重塑（#47）
- 事件：三连交付（net-doctor `3d78935`/`1bc97d4`/`8b0daa0` / 主仓库 `d04ecf1`/`66cffed`/`650cd1e`）：①包长档位单逗号框→4 个独立文本框（校验/去重/行内红字，存储格式不变零后端）②iperf3 时长档位 10/15/30→10/15/30/60/120/180/300 + 自定义输入（后端 clamp 120→300 对齐，hard 标志区分硬错误/截断提示）③压测分阶段进度条动画（六段加权 / accent 流动动画 / 终态三色，纯前端零后端）。E2E 234→252。
- 意义：压测配置可用性与过程可视化升级；确立「前端自由输入、后端硬约束 + 软提示」的参数校验模式（clamp 对齐 + hard 标志）。
- 考证：net-doctor 提交 `3d78935`（2026-09-10 21:41）、`1bc97d4`（22:04）、`8b0daa0`（22:13）；主仓库提交 `d04ecf1`（21:41）、`66cffed`（22:05）、`650cd1e`（22:13）；GitLab 推送 `ff4a16a`（22:15）；E2E 234→252 来源：会话记忆（2026-09-10）。

#### 2026-09-10 · route_nodes v5 基线勘误与扩充（#50，数据维护）
- 事件：172.17.90.0/24 zone 勘误「核心交换机」→「终端区」（网关设备=核心交换机，gw_ip 保留）+ 新增 115.236.74.73/32 互联网出口（杭州）；控制台 + 终端端点双链路校验；v4→v5。
- 意义：路由基线数据质量修正（zone 语义纠错）+ 出口节点扩充，tracert 区域标注准确性提升。
- 考证：服务端数据维护无 git 提交（会话记忆，2026-09-10 晚~09-11 晨批次内）；v4→v5 与双链路校验来源：会话记忆。

#### 2026-09-11 · AI 聚合分析 60s 通道热修复（#49）
- 事件：conflict-ai-reanalyze 被通用 15s 超时层掐断（LLM 实测 30.3s）→ 专用 60s 长超时通道（对齐 ndAiPostDiagnose 先例）（net-doctor `b4ec338` / 主仓库 `16ad87f`）；精确注入验证（__apiFetchTimeout 400ms + 慢桩 900ms）。
- 意义：「AI 类长耗时请求专用超时通道」模式第二次落地，通用 15s 层的例外清单化，杜绝 LLM 真实耗时被误杀。
- 考证：net-doctor 提交 `b4ec338`（2026-09-11 08:31）；主仓库提交 `16ad87f`（08:31）、指针 `a84af58`（08:32）；E2E 254/254 来源：会话记忆（2026-09-11）。

#### 2026-09-11 · AI 智能诊断卡排版对齐终端概览（#51）
- 事件：应用户反馈 AI 卡字体排版与概览三卡不统一，net-doctor `00e7a06` / 主仓库 `5d8af64` 将 AI 卡对齐 hm-card 设计语言：卡头 h3 13.5px/600/accent-cyan、卡体 padding 15px 17px、说明句 nd-summary 规格、问题概述行改 hm-row 左标签右值式（字数计数移右）、辅助文字 12px、textarea 对齐 settings-input、nd-empty 空态全局统一；全部作用域限 .nd-ai-card 不波及其它卡；独立页无 hm 家族补充等效类同步。
- 意义：首次将概览卡设计语言跨卡复用；「样式无成文规范」问题显性化，直接催生 STYLE.md 设计规格（#52）。
- 考证：net-doctor 提交 `00e7a06`（2026-09-11 08:46）；主仓库提交 `5d8af64`（08:46）；E2E 259/259（含 computed style 实测比对：AI 卡标题字号===概览卡标题）来源：会话记忆（2026-09-11）。

#### 2026-09-11 · 定名回退「网络排障」+ 五模块卡对齐 + STYLE.md 设计规格诞生（#52）
- 事件：三合一交付（net-doctor `52c06cd` / 主仓库 `823caf4`，指针 `1a000f3`）：①**定名回退**「网络监测配置」→「网络排障」——9 处用户可见文案反向回退、内部标识零改动，ADR-012 修订（2026-09-11）：网络排障为最终定名（用户定），与 #39 更名构成反转闭环；②五模块卡（配置核查/IP 冲突/连通性/路由追踪/网络压测）+ 中心平台状态条加 nd-styled 作用域类对齐 AI 卡基线（nd-kv 键值行式化/nd-table 统一/实现细节句清理改通俗语）；③**STYLE.md 设计规格诞生**（net-doctor/docs/STYLE.md）——结构/字号/间距/行式/按钮/空态/表格/徽章八类规格登记，新建任何卡片必须遵循、交付检查单含样式符合性自检（不合规=门禁红）。E2E 262/262（五卡卡头字号/卡体 padding===AI 卡 + 实现细节零残留断言）。
...
- 档案指针（2026-09-18 晚补，用户点名专项文档）：功能开发细节全记录（失败路线 / 两端实现 / 通道实证 / 方法论红线 / 排障手册）见 **docs/WOL_TIMED_BOOT.md**。
