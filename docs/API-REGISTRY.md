...
- **状态**：在用
- **登记记录**：2026-09-09，代码实证（server-platform-dev 下发，commit 4d2924b）> 更新 2026-09-19：**生产配置取值变更（main 下发）**——llm.model=DeepSeek-V4.1（主）/ llm.model_fallback=Qwen3.6（备），端点契约无变化（详见 EXT-001 更新行）。**运维 CLI 锚**：`/data/terminal-platform/app/server/config_cli.py`（set/get/list 子命令，SSH 服务器侧，set --stdin 加密落库）——**已知缺陷（2026-09-19，待修复）**：`list` 子命令报 AttributeError：`SettingsStore.DEFAULTS` 不存在（DEFAULTS 为 settings.py 模块级常量、非类属性）；get/set 正常；修复建议 `from settings import DEFAULTS` 后遍历。待 server-platform-dev 修复后知会消项 > 更新 2026-09-19（稍后）：**消项——缺陷已修复并部署（server-platform-dev）**，修复较原建议更彻底：改用 SettingsStore 已有 `all_masked()`（合并「settings 表实际键」+「DEFAULTS 预置键」、内部自带敏感脱敏；原建议遍历 DEFAULTS 会**漏列**不在 DEFAULTS 中的键——实测 llm.model_fallback/llm.api_key/switch.default_password/ftp.password/iperf.server_ip 等即属此类），list 分支过滤 .updated_at 后排序输出（config_cli.py:70-72 + 模块头注释 :22-25 说明为何走 all_masked；本地源码已改，py_compile 通过；**源码 commit a7f36b7**——server-platform 仓库 fix(config_cli): list subcommand via SettingsStore.all_masked()，提交信息含修复人与反例说明）。部署与验证（172.17.5.215）：远端备份 config_cli.py.bak.20260919_0100；`list` 实测 26 键完整（llm.model=DeepSeek-V4.1 / llm.model_fallback=Qwen3.6 / llm.api_key=sk-****xJJH / switch.default_password 已脱敏）；回归 get llm.model、get llm.api_key --mask、get llm.api_key（无 --mask 正确拒绝）、get wol.pinned_relay（未设置 → unset）全通过；服务 active / health=200。**反例存档**：server-platform/tools/_fix_conpass.py 曾用 sed 直改远端 config_cli.py 修同一缺陷但未回改源码，后续部署覆盖导致缺陷复发——「运维脚本绕过源码」典型反例，与「代码是唯一事实来源」铁律呼应，留作部署纪律培训例（远端变更必须同步回源码）
