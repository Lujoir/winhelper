...
> **scope 变更（ADR-032 双端口白名单）**：terminal 口（18443）放行追加 `/api/v1/client/manifest` 与 `/download/*`（`_scope_allowed` api.py:216-220，终端自动更新拉包用）；console 口经静态 GET 规则天然可用 `/download/*`；legacy 口全量。dispatch docstring 注释曾未同步（原写仅 terminals/ai/analyze）——**已消项 2026-09-17**：9ce055f 同步注释并在 docstring 头部标注「白名单以 _scope_allowed 为准」（api.py:232，代码实证），台账原「以 _scope_allowed 实现为准」标注保留作锚点。
