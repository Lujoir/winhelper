...
- **鉴权**：无（本机进程内）
- **请求参数**：无
- **响应**：`{"success":true,file_count,dir_count,volumes,partial_volumes:[...],...}`（**partial_volumes 透出**为 e39ec89 补齐项——卷扫描未完成段列表，供重建判定）
- **代码出处**：bridge.py:155；file-search/search_service.py `handle_fs_status`(176)
- **状态**：在用（前端 UI 已不再消费，端点保留；partial_volumes 随 e39ec89 同步补齐）
- **登记记录**：2026-09-17，代码实证（漏登补登同 BRG-070）> 更新 2026-09-17（晚 2）：partial_volumes 字段随主仓同步补齐（e39ec89 实证 :180-188）
...
- **代码出处**：server-platform 5f0ba2f（api.py :491 回执钩子 / :1001 白名单 / :1037 入队）；**终端 handler 由 power-control-dev 4.1.2 交付（当前未在位——终端收到 pc_diag 将按 UPL-009 语义回 unknown_command_type，生产联调 diag#1 是在已带 handler 的 4.1.x 构建上完成）**
- **状态**：在用（服务端已生产部署验证 diag#1 completed；终端执行面待 4.1.2 交付后全量闭环）
- **登记记录**：2026-09-17，代码实证（server-platform-dev 实施，commit 5f0ba2f/ADR-040 增补，按常设约定显式列清单知会，api-registrar-dev 复核登记）
