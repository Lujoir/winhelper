...
- **登记统计**：
  ...
  - 三、终端↔平台协议（UPL）：14 条（2026-09-16 新增桌面管控契约 3 条 UPL-011~013，契约冻结/服务端未实现；2026-09-17 新增命令 UPL-014 pc_apply_policy，主仓 a099448）
...
> ⚠️ **主仓副本待同步（2026-09-17 实证）**：主仓 file_search.py FS_SORTABLE(:24-29) 无 `ext` 值、无 mtime_from/mtime_to（仅 date_from/date_to :117-122）——**主应用运行态 ext 排序/mtime 参数暂不可用**，待 net-doctor-dev 同步 894f20a 增量后转全量在用；BRG-066~069（三大改造）直写事件同期发现，主仓同步纪律需收紧。
...
- **响应**：`{"success":true,"q":...,"total":N,"count":N,"results":[{name,path,size,date_modified}]}`（≤200）；引擎不可用如实提示；q 与 ext: 全空 400 语义
- **代码出处**：bridge.py:42/:154；权威实现 file-search/search_service.py `handle_fs_query`(89)/`FS_SORTABLE`(24-30)/`_parse_date_param`(34)；主仓副本 file_search.py `handle_fs_query`(88)（**待同步**：无 ext 排序/mtime_*/date 别名已具备）
- **状态**：在用（**ext 排序/mtime 参数：子仓已实施、主仓副本待同步**）
- **登记记录**：2026-09-17，代码实证（漏登补登：路由 c37cf97/5f32dbe/17f506c 批次未随批登记；894f20a 参数增强经 net-doctor-dev 按常设约定知会，api-registrar-dev 对账发现整组漏登一并首登）

#### BRG-071 检索状态 `GET /api/filesearch/status`
- **用途**：索引器状态查询（索引库存在性/条数等；7a88b91 后前端「检索状态卡」已移除不再消费其渲染，端点仍正常服务）
- **鉴权**：无（本机进程内）
- **请求参数**：无
- **响应**：`{"success":true,...索引状态字段}`（以 handler 为准）
- **代码出处**：bridge.py:155；file-search/search_service.py `handle_fs_status`(176)
- **状态**：在用（前端 UI 已不再消费，端点保留）
