...
- **登记统计**：
  ...
  - 二、终端本地桥接 API（BRG）：58 条（netdoctor 19 条：10 条 2026-09-09 合入转「在用」commit 628c210；AI 诊断 2 条 commit a892f62/bd965ae；冲突检测本地转发 3 条 commit b344bbc/40abd3f；路由追踪 AI 分析 + AI 诊断历史持久化 4 条 2026-09-11 登记，commit c4495bc/916e791）
  - 三、终端↔平台协议（UPL）：10 条
...
- **鉴权**：HRESS 签名（HMAC-SHA1 → base64 → urlencode；`Authorization` 头或 URL 参数两式）——签名串 `AK\nexpires\nMETHOD\nContent-MD5\nCanonicalizedResource`；`Content-MD5 = base64(md5(请求体二进制))`（必须对发送字节原文计算）；`CanonicalizedResource`=路径，仅 query 传参时子资源按字典序拼接；expires=服务器时间+300s（实测时钟偏移可忽略）
...
- **调用方式**：仅服务端发起（规划 `server-platform/server/huorong.py` HuorongClient：签名/分页/errno 归一/重试/任务门禁；终端与前端不直连火绒）。示意（集成后）：
  `curl -X POST https://<hr-host>:8080/api/group/_list -H "Content-Type: application/json" -H "Authorization: HRESS <signature>" -d '{}'`
...
- **状态**：调研中——连通性已验证（HTTP 200 + 标准信封）；**签名认证未通过**（15 变体全部 errno=1，签名实现已覆盖文档全部歧义点，最可能凭据对被拒，待用户核对 AK/Secret 成对性与启用状态后复验）；任务类接口**零调用**（红线）
- **登记记录**：2026-09-15，huorong-dev 下发（官方 API v1 文档解析 + 实测，方案文档与客户端雏形已核对入仓）——**待代码实证**（P1 `huorong.py` 正式化落地后对账补证）
