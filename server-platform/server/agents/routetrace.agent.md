<!--
  ...
  源文件  : ai-agent/agents/center/routetrace.agent.md
  发布时间: 2026-09-20 11:53:02
  内容指纹: 3247fc7390b1fd19   （源文件正文 SHA256 前 16 位，校验用）
...
| **待迁移项** | ①~~修复 `data`/`context` 键名不一致~~ ✅ **已完成**（2026-09-20 核实：客户端两副本均已发 `context` 键并注明 ADR-031 契约冻结，此前台账记录的欠账已过时）；②~~超时统一收紧~~ ✅ **已完成**（ADR-009）；③~~身份"企业"→"医院"~~ ✅ **已完成**；④~~注入隐私前缀~~ ✅ **已完成**；⑤机制项（记忆 / 推理留痕）见 AUDIT P1 |
